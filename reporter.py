...
class Reporter:
    ...
    def remember(self, n_steps, snake_len, reward):
        self._memory.append((n_steps, snake_len, reward))
        self.n_episodes += 1

    def get_report_str(self):
        arr = np.array(self._memory)
        means = np.mean(arr, axis=0)
        report_format = 'Episodes {}/{} | avg steps: {}, avg len: {}, ' \
                        'avg reward: {} (last {} games)'
        report_data = (
            self.n_episodes, self.max_episodes, means[0], means[1],
